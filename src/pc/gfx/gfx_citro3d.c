...
struct video_buffer {
    ...
    uint8_t stride;
    uint32_t offset;
    shaderProgram_s shader_program; // actual vertex shader
...
static struct video_buffer video_buffers[32];
...
static void update_shader(bool swapInput01)
{
    ...
    if (new_prg->cc_features.num_inputs == 2)
    {
        C3D_TexEnvInit(env);
    ...
    C3D_TexEnvInit(env);
    C3D_TexEnvColor(env, 0);
    if (new_prg->cc_features.opt_alpha && !new_prg->cc_features.color_alpha_same)
    {
        // RGB first
        if (new_prg->cc_features.do_single[0])
        {
            C3D_TexEnvFunc(env, C3D_RGB, GPU_REPLACE);
            C3D_TexEnvSrc(env, C3D_RGB, getTevSrc(new_prg->cc_features.c[0][3], swapInput01), 0, 0);
            if (new_prg->cc_features.c[0][3] == SHADER_TEXEL0A)
                C3D_TexEnvOpRgb(env, GPU_TEVOP_RGB_SRC_ALPHA, GPU_TEVOP_RGB_SRC_COLOR, GPU_TEVOP_RGB_SRC_COLOR);
            else
                C3D_TexEnvOpRgb(env, GPU_TEVOP_RGB_SRC_COLOR, GPU_TEVOP_RGB_SRC_COLOR, GPU_TEVOP_RGB_SRC_COLOR);
        }
        else if (new_prg->cc_features.do_multiply[0])
        {
            C3D_TexEnvFunc(env, C3D_RGB, GPU_MODULATE);
            C3D_TexEnvSrc(env, C3D_RGB, getTevSrc(new_prg->cc_features.c[0][0], swapInput01),
                                        getTevSrc(new_prg->cc_features.c[0][2], swapInput01), 0);
            C3D_TexEnvOpRgb(env,
                new_prg->cc_features.c[0][0] == SHADER_TEXEL0A ? GPU_TEVOP_RGB_SRC_ALPHA : GPU_TEVOP_RGB_SRC_COLOR,
                new_prg->cc_features.c[0][2] == SHADER_TEXEL0A ? GPU_TEVOP_RGB_SRC_ALPHA : GPU_TEVOP_RGB_SRC_COLOR,
                GPU_TEVOP_RGB_SRC_COLOR);
        }
        else if (new_prg->cc_features.do_mix[0])
        {
            C3D_TexEnvFunc(env, C3D_RGB, GPU_INTERPOLATE);
            C3D_TexEnvSrc(env, C3D_RGB, getTevSrc(new_prg->cc_features.c[0][0], swapInput01),
                                        getTevSrc(new_prg->cc_features.c[0][1], swapInput01),
                                        getTevSrc(new_prg->cc_features.c[0][2], swapInput01));
            C3D_TexEnvOpRgb(env,
                new_prg->cc_features.c[0][0] == SHADER_TEXEL0A ? GPU_TEVOP_RGB_SRC_ALPHA : GPU_TEVOP_RGB_SRC_COLOR,
                new_prg->cc_features.c[0][1] == SHADER_TEXEL0A ? GPU_TEVOP_RGB_SRC_ALPHA : GPU_TEVOP_RGB_SRC_COLOR,
                new_prg->cc_features.c[0][2] == SHADER_TEXEL0A ? GPU_TEVOP_RGB_SRC_ALPHA : GPU_TEVOP_RGB_SRC_COLOR);
        }
        // now Alpha
        C3D_TexEnvOpAlpha(env,  GPU_TEVOP_A_SRC_ALPHA, GPU_TEVOP_A_SRC_ALPHA, GPU_TEVOP_A_SRC_ALPHA);
        if (new_prg->cc_features.do_single[1])
        {
            C3D_TexEnvFunc(env, C3D_Alpha, GPU_REPLACE);
            C3D_TexEnvSrc(env, C3D_Alpha, getTevSrc(new_prg->cc_features.c[1][3], swapInput01), 0, 0);
        }
        else if (new_prg->cc_features.do_multiply[1])
        {
            C3D_TexEnvFunc(env, C3D_Alpha, GPU_MODULATE);
            C3D_TexEnvSrc(env, C3D_Alpha, getTevSrc(new_prg->cc_features.c[1][0], swapInput01),
                                          getTevSrc(new_prg->cc_features.c[1][2], swapInput01), 0);
        }
        else if (new_prg->cc_features.do_mix[1])
        {
            C3D_TexEnvFunc(env, C3D_Alpha, GPU_INTERPOLATE);
            C3D_TexEnvSrc(env, C3D_Alpha, getTevSrc(new_prg->cc_features.c[1][0], swapInput01),
                                          getTevSrc(new_prg->cc_features.c[1][1], swapInput01),
                                          getTevSrc(new_prg->cc_features.c[1][2], swapInput01));
        }
    }
    else
    {
        ...
        {
            C3D_TexEnvFunc(env, C3D_Both, GPU_REPLACE);
            C3D_TexEnvSrc(env, C3D_Both, getTevSrc(new_prg->cc_features.c[0][3], swapInput01), 0, 0);
            if (new_prg->cc_features.c[0][3] == SHADER_TEXEL0A)
                C3D_TexEnvOpRgb(env, GPU_TEVOP_RGB_SRC_ALPHA, GPU_TEVOP_RGB_SRC_COLOR, GPU_TEVOP_RGB_SRC_COLOR);
            else
                C3D_TexEnvOpRgb(env, GPU_TEVOP_RGB_SRC_COLOR, GPU_TEVOP_RGB_SRC_COLOR, GPU_TEVOP_RGB_SRC_COLOR);
        ...
        else if (new_prg->cc_features.do_multiply[0])
        {
            ...
            C3D_TexEnvSrc(env, C3D_Both, getTevSrc(new_prg->cc_features.c[0][0], swapInput01),
                                         getTevSrc(new_prg->cc_features.c[0][2], swapInput01), 0);
            C3D_TexEnvOpRgb(env,
                new_prg->cc_features.c[0][0] == SHADER_TEXEL0A ? GPU_TEVOP_RGB_SRC_ALPHA : GPU_TEVOP_RGB_SRC_COLOR,
                new_prg->cc_features.c[0][2] == SHADER_TEXEL0A ? GPU_TEVOP_RGB_SRC_ALPHA : GPU_TEVOP_RGB_SRC_COLOR,
    ...
    if (!new_prg->cc_features.opt_alpha)
    {
                                    ...
                                    bool has_color, bool has_color2)
{
    ...
    cb->ptr = linearAlloc(512 * 1024); // sizeof(float) * 10000 vertexes * 10 floats per vertex?
    // Configure buffers
...
{
    if (current_buffer->offset * current_buffer->stride > 512 * 1024 / 4)
    {
